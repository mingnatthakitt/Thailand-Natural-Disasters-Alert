    ...
    <div className="flex flex-col h-screen w-screen overflow-hidden" style={{ background: 'var(--background)' }}>
      ...
      <header className="glass-panel border-b border-white/[0.06] px-5 py-3 flex items-center gap-4 shrink-0 z-20">
        ...
        <div className="hidden md:flex items-center gap-4 ml-auto mr-4">
          <StatChip icon={<Flame size={12} />} label="Active Fires" value={stats.fires} color="orange" />
          <StatChip icon={<Activity size={12} />} label="Earthquakes (7d)" value={stats.quakes} color="red" />
          <StatChip icon={<Activity size={12} />} label="Max Richter" value={stats.maxMag > 0 ? `M${stats.maxMag.toFixed(1)}` : '—'} color="cyan" />
          <StatChip
            icon={<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#94A3B8" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M12 2v20M2 12h20M5.5 5.5l13 13M18.5 5.5l-13 13" /></svg>}
            label="Active Storms"
            value={stats.storms}
            color="slate"
          />
      ...
      <div className="flex flex-1 overflow-hidden relative">
              ...
              {(['all', 'wildfire', 'earthquake', 'storm'] as const).map((t) => (
                <button
                  ...
                  className={`
                    ...
                    ${typeFilter === t
                      ? t === 'wildfire' ? 'bg-orange-500/20 text-orange-400 border border-orange-500/30'
                      : t === 'earthquake' ? 'bg-red-500/20 text-red-400 border border-red-500/30'
                      : t === 'storm' ? 'bg-slate-500/20 text-slate-300 border border-slate-500/30'
        ...
        <main className="flex-1 relative">
          ...
          <div className="absolute top-4 right-4 z-20 region-overlay pointer-events-none">
            ...
            <div className="text-[10px] mt-1 opacity-70">4°N–22.5°N, 95°E–107.5°E</div>
          </div>
          ...
          <div className="absolute bottom-4 left-4 z-20 glass-panel rounded-lg px-3 py-2.5 space-y-1.5 pointer-events-none">
            ...
            <div className="flex items-center gap-2 text-[11px] text-slate-300">
              <span className="w-4 h-4 rounded-full bg-[#1E293B] border border-slate-400 flex items-center justify-center">
